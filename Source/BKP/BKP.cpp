...
void BKP::set_data(Backup_Data datax, uint16_t data) {
    ...
    // Check clock enabled since no read/write operations are used
...
void BKP::set_rtc_output_calibration_enable(bool enable) {
    write_bit(*this, BKP_Regs::OCTL, static_cast<uint32_t>(OCTL_Bits::COEN), enable ? Set : Clear, true);
}

void BKP::set_rtc_output_signal_enable(bool enable) {
    write_bit(*this, BKP_Regs::OCTL, static_cast<uint32_t>(OCTL_Bits::ASOEN), enable ? Set : Clear, true);
}

void BKP::set_rtc_output_pulse(Output_Pulse pulse) {
    write_bit(*this, BKP_Regs::OCTL, static_cast<uint32_t>(OCTL_Bits::ROSEL), (pulse == Output_Pulse::SECOND_PULSE) ? OUTPUT_SECOND_PULSE : Clear, true);
}

void BKP::set_rtc_clock_divider(Clock_Divider divider) {
    write_bit(*this, BKP_Regs::OCTL, static_cast<uint32_t>(OCTL_Bits::CCOSEL), (divider == Clock_Divider::DIV_1) ? CLOCK_DIV_1 : Clear, true);
}

void BKP::set_rtc_clock_calibration_type(Calibration_Type type) {
    write_bit(*this, BKP_Regs::OCTL, static_cast<uint32_t>(OCTL_Bits::CALDIR), (type == Calibration_Type::SPEED_UP) ? CLOCK_SPEED_UP : Clear, true);
}

void BKP::set_rtc_calibration_value(uint8_t value) {
    write_bit(*this, BKP_Regs::OCTL, static_cast<uint32_t>(OCTL_Bits::RCCV), static_cast<uint32_t>(value), true);
}

void BKP::set_tamper_detection_enable(bool enable) {
    write_bit(*this, BKP_Regs::TPCTL, static_cast<uint32_t>(TPCTL_Bits::TPEN), enable ? Set : Clear, true);
}

void BKP::set_tamper_level(Tamper_Level level) {
    write_bit(*this, BKP_Regs::TPCTL, static_cast<uint32_t>(TPCTL_Bits::TPAL), static_cast<uint32_t>(level), true);
}

void BKP::set_tamper_interrupt_enable(bool enable) {
    write_bit(*this, BKP_Regs::TPCS, static_cast<uint32_t>(TPCS_Bits::TPIE), enable ? Set : Clear, true);
}

bool BKP::get_flag(Status_Flags flag) {
    return (read_bit(*this, BKP_Regs::TPCS, static_cast<uint32_t>(flag), true) != Clear);
}

void BKP::clear_flag(Clear_Flags flag) {
    write_bit(*this, BKP_Regs::TPCS, static_cast<uint32_t>(flag), Set, true);
}
